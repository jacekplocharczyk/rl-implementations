...
class Actions(space_aggregator.SpaceAggregator):
    ...
    def check_actions(
        self, taken_discrete_actions: List[int], taken_continuous_actions: List[float]
    ):

        for i in range(len(taken_discrete_actions)):
            self.check_discrete_action(
                taken_discrete_actions[i], self.discrete_space[i]
            )

        for i in range(len(taken_continuous_actions)):
            self.check_continuous_action(
                taken_continuous_actions[i], self.continuous_space[:, i]
            )

    @staticmethod
    def check_discrete_action(taken_action: int, allowed_actions: int) -> None:
        error = (
            f"Not allowed discrete action: {taken_action} for range({allowed_actions})"
        )
        if taken_action not in range(allowed_actions):
            raise ValueError(error)

    @staticmethod
    def check_continuous_action(taken_action: int, allowed_range: np.array) -> None:
        error = (
            f"Not allowed continuous action: {taken_action} for range {allowed_range})"
        )
